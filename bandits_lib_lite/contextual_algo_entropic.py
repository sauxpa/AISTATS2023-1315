...
def EntropicRiskUCB(
    ...
    support_lo: float = -1.0,
    bound_theta: float = 1.0,
    bound_features: float = 1.0,
    warmup: int = 1,
    verbose: bool = True,
):
    """EntropicRiskUCB with exact entropic risk calculation at each step
    ...
    def entropic_loss(y: float, z: float) -> float:
        return z + 1 / gamma * (np.exp(gamma * (y - z)) - 1)

    def entropic_loss_hessian(y: float, z: float) -> float:
        return gamma * np.exp(gamma * (y - z))

    diam = support_hi - support_lo
    m = 2 * gamma * np.exp(-gamma * diam)
    M = 2 * gamma * np.exp(gamma * diam)
    kappa = M / m
    ...
    for t in tqdm(range(T), disable=not verbose):
        ...
        # Update entropic risk regression
        A = np.zeros((d, K))
        ...
        def empirical_entropic_loss(theta):
            return (
                np.sum(
                    [
                        entropic_loss(
        ...
        if ret.success:
            theta_hat = ret.x
        # else:
        # raise Exception("Entropic risk calculation failed...")

        D2_loss = [
            entropic_loss_hessian(
        ...
        # V0 += np.outer(features_history[:, t], features_history[:, t])
...
def EntropicRiskOGDUCB(
    ...
    """EntropicRiskOGDUCB with OGD entropic risk calculation at each step
    for a fixed confidence level delta.
    exploration_bonus_scale: multiplicative factor in the
        UCB exploration bonus.
    verbose: whether to display tqdm.
    ...
    def entropic_loss_grad(y: float, z: float) -> float:
        return 1 - np.exp(gamma * (y - z))

    def entropic_loss_hessian(y: float, z: float) -> float:
        return gamma * np.exp(gamma * (y - z))

    diam = support_hi - support_lo
    m = 2 * gamma * np.exp(-2 * gamma * diam)
    M = 2 * gamma * np.exp(2 * gamma * diam)
    kappa = M / m
